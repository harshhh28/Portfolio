...
flowchart TD
  ...
  subgraph FE[Frontend - Admin UI]
    A1[Enter NL Query]
    A2[Send Request]
    A3{Response OK?}
    A4[Show Clarification]
    A5[Display Results]
  end
  ...
  subgraph BE[Backend - Node.js]
    B1[Validate Request]
    B2[Process Query]
    B3[Call Intent Planner]
    B4[Validate Response]
    B5{Clarification Needed?}
    B6[Build DB Query]
    B7[Fetch Users]
    B8[Return Response]
  end

  subgraph GA[GenAI Service]
    C1[Intent Planner API]
    C2[Generate Filters]
    C3[Return Structured Response]
  end
